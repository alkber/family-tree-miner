...
/**
 * Base class to find a children of a subject wrt sex.
 *
 * @author Althaf K Backer <althafkbacker@gmail.com>
 */
public class Child extends Relation {

	protected final boolean sex;

	public Child(Subject subject, boolean sex) {
	...
	public List<Subject> find() {
		...
		List<Subject> children = subject.getChildren();

		for (Subject currentChild : children) {

			if (currentChild.getSex() == sex) {

				childrenBySex.add(currentChild);
			}
		}
