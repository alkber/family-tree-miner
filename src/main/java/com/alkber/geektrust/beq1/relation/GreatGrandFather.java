...
public class GreatGrandFather extends GreatGrandParent {

	public GreatGrandFather(final Subject subject) {
	...
	protected void fillGreatGrandParents(ArrayList<Subject> greatGrandParents) {
		...
		for(Subject current : grandParentsOfSubject) {
			...
			// grandfather's father or grandmother's father
			if (current.getFather() != null) {

				greatGrandParents.add(current.getFather());
			}
		}
	}
}
